...
import (
	...
	contextcmd "github.com/rohithmahesh3/taskforge-cli/cmd/context"
	"github.com/rohithmahesh3/taskforge-cli/internal/output"
	"github.com/spf13/cobra"
)

const (
	markerStart = "<!-- TASKFORGE_TASK_MANAGEMENT_CLI_START -->"
	markerEnd   = "<!-- TASKFORGE_TASK_MANAGEMENT_CLI_END -->"
)
...
var InjectCmd = &cobra.Command{
	Use:   "inject",
	Short: "Inject taskforge context into agent files",
	Long: `Inject taskforge context documentation into agent files (AGENTS.md, GEMINI.md, CLAUDE.md, CURSOR.md).
...
- Reference to taskforge context for full documentation
- Configuration options

The section is marked with HTML comments so it can be automatically updated.

Examples:
  taskforge inject                    # Update all default agent files
  taskforge inject --file AGENTS.md   # Update specific file
  taskforge inject --dry-run          # Show what would change
  taskforge inject --force            # Force update even if unchanged`,
	RunE: runInject,
}
